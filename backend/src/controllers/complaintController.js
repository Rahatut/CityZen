...
exports.getRecommendedAuthorities = async (req, res) => {
  try {
    ...
    console.log('🔍 Recommend Authorities Request:', { categoryId, latitude, longitude });
    ...
    console.log('📋 Companies for category:', companiesForCategory.length);

    if (companiesForCategory.length === 0) {
      console.log('❌ No companies found for category:', categoryId);
      return res.json([]);
    }
    ...
    console.log('📍 Company areas found:', companyAreas.length);
    companyAreas.forEach(area => {
      const dist = haversineDistance(userLat, userLon, area.latitude, area.longitude);
      console.log(`  - Area: ${area.name}, Distance: ${dist.toFixed(2)}km, Radius: ${area.radius}km, Match: ${dist <= area.radius}`);
    });
    ...
    console.log('✅ Relevant companies:', relevantCompanies.size);

    if (relevantCompanies.size === 0) {
      console.log('❌ No companies within radius');
      return res.json([]);
    }
    ...
    console.log('✅ Recommended authorities:', recommendedAuthorities.length);
    recommendedAuthorities.forEach(auth => console.log(`  - ${auth.name}`));
