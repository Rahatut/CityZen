...
export default function SubmitComplaintDetailsScreen({ navigation, onLogout, darkMode, toggleDarkMode, route }) {
    ...
    const runAiDetection = async (imageUri) => {
        ...
        try {
            console.log("Sending AI detection request to:", `${API_URL}/api/ai/detect-pothole`);
            const controller = new AbortController();
            const timeoutId = setTimeout(() => controller.abort(), 30000); // 30 second timeout

            const res = await fetch(`${API_URL}/api/ai/detect-pothole`, {
                method: "POST",
                body: formData,
                signal: controller.signal,
                // DO NOT set Content-Type header - let the browser set it for multipart/form-data
            });
            ...
            if (!res.ok) {
                ...
                console.error("AI Detection Error Response:", errorData);
                throw new Error(errorData.message || `AI detection failed with status ${res.status}`);
            }

            const data = await res.json();
            console.log("AI Detection Success:", data);
            return data;
        } catch (err) {
            ...
            console.error("Error Details:", {
                message: err.message,
                code: err.code,
                apiUrl: `${API_URL}/api/ai/detect-pothole`
            });
            
            let message = "Failed to analyze image";
            if (err.name === "AbortError") {
                message = "Request timeout - image analysis took too long";
            } else if (err.message?.includes("Network")) {
                message = "Network error - check your connection";
            } else if (err.message?.includes("Failed to fetch")) {
                message = "Could not reach backend server. Is it running?";
            }
    ...
    const handleImagePick = async () => {
        try {
            ...
            if (result.assets?.length > 0) {
                ...
                // Only update location if it's the first image or location not set
                ...
                <View style={[styles.card, darkMode && styles.cardDark]}>
                    ...
                    <View style={styles.uploadRow}>
                        <TouchableOpacity 
                            ...
                            style={[styles.uploadBtn, images.length > 0 ? styles.uploadBtnSmall : null, errors.image && styles.errorBorder]}
                            activeOpacity={0.7}
                        >
